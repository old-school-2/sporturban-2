...
//            break;
        ...
        case 'главная' :
            $state   = $type;
//            $array_buttons = type_sport_button($array_buttons);
            $sportzone = get_sportzone_array($protocol, $data, get_geo($protocol, $data));
            $array_buttons = sportzone_button($array_buttons, $sportzone);
...
//            array_unshift($array_buttons, array('name' => '🏠' . $settings['adress'], 'text' => 'Адрес'));
...
//                $content = get_content_text($protocol,  'Ещё мероприятия:', null, $data);
               $content = get_content_list($protocol, 'Ещё площадки:', $buttons, $data,  $events, $log);
            }
            break;
...
//    if ($state == 'адрес') {
        ...
        if ( $adress_name != '') {
            ...
            $sportzone = get_sportzone_array($protocol, $data, $geo);
            $array_buttons = sportzone_button($array_buttons, $sportzone);
            array_unshift($array_buttons, array('name' =>  $adress_name, 'text' => 'Адрес'));

            $buttons = get_buttons($protocol,  $array_buttons);

            if ($sportzone != null){
                $content = get_content_text($protocol,  "В радиусе 1 км от " .  $adress_name . " найдено " . count( $sportzone ) . " спортивных объектов:", $buttons, $data);//count($sportzone)
            }else{
                $content = get_content_text($protocol,  "В радиусе 1 км от " . $adress_name . " нет спортивных зон, задайте другой адрес", $buttons, $data);//count($sportzone)
            }
...
function s_type_sport($protocol, $data, $type, $array_buttons, $character, $state, $text, $log){
    if  ($text == 'любой' ){
        ...
        $sport_id = check_sport($text);
    }

    if ($sport_id != null ){
        $events = get_events_array($protocol, $data, $sport_id, $log['offset'] );
        $array_buttons = next_button($protocol, $data, $array_buttons, $events );

        if ( $events  == null ){
            $buttons = get_buttons($protocol,  $array_buttons);
            $content = get_content_text($protocol,  'Ничего не нашлось, укажите другой Адрес, или Вид спорта', $buttons, $data);
        }else{
            $buttons = get_buttons($protocol,  $array_buttons);
            $content = get_content_list($protocol, 'Спортивные зоны', $buttons, $data,  $events, $log);
        }
